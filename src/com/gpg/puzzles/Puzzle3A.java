...
/**
 ...
 and consist of a single rectangle with edges parallel to the edges of the fabric.
 ...
 The correct answer is 115304 with puzzle3.dat
...
public class Puzzle3A extends Puzzle3Base
{
    protected void SolveMe(ArrayList<String> inputData) {
        ...
        Map<String, Integer> takenAreas = new Hashtable<String, Integer>(1000);
        Map<String, String> takenClaims = new Hashtable<String, String>(1000);

        System.out.println("Puzzle 3A Solver");
        for (String data : inputData) {
            if (isStringNullOrEmpty(data)) {
                continue;
            }

            RecordClaim(data, takenAreas, takenClaims);
        }

        Integer multipleClaimedSquareInches = 0;

        for (String id : takenAreas.keySet()) {
            if (takenAreas.get(id) > 1) {
                multipleClaimedSquareInches++;
            }
        }
        System.out.println(String.format("The number of square inches with multiple claims is %d", multipleClaimedSquareInches));

        // Solve part B using the data we have collected.
        Puzzle3B solverB = new Puzzle3B();
        solverB.SolveMe(takenAreas, takenClaims);
