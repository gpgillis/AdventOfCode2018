...
/**
 ...
 and three of them contain a letter which appears exactly three times. Multiplying these together produces a
 checksum of 4 * 3 = 12.

 What is the checksum for your list of box IDs?*

 The correct answer is 6916 using puzzle2.dat

 */
public class Puzzle2A extends Puzzle2Base {

    protected void SolveMe(ArrayList<String> inputData) {
        Integer twoRepeatCount = 0;
        Integer threeRepeatCount = 0;
        Map<Character, Integer> found = new HashMap<Character, Integer>();

        for (String boxId : inputData) {
            System.out.println(String.format("Working on box id %s", boxId));

            found.clear();

            for (int i = 0; i < boxId.length(); i++) {
                Character c = boxId.charAt(i);
//                System.out.println(String.format("Character found is %s", c));
                Integer count = 1;

                if (found.containsKey(c)) {
//                    System.out.println(String.format("Found an instance of %s", c));
                    count = found.get(c) + 1;
                }
...
//            for (Character key : found.keySet()) {
//                System.out.println(String.format("Key %s contains count %d", key, found.get(key)));
//            }
        ...
        Integer checksum = twoRepeatCount * threeRepeatCount;

        System.out.println(String.format("The two repeat = %d, three repeat = %d, checksum = %d", twoRepeatCount, threeRepeatCount, checksum));
